...
public final class Picture implements ActionListener {
    ...
    /**.
      ...
      */
    public Picture(final int w, final int h) {
        if (w  < 0) {
    ...
    public Picture(final Picture pic) {
        ...
        isOriginUpperLeft = pic.isOriginUpperLeft;
        for (int col = 0; col < width(); col++) {
            for (int row = 0; row < height(); row++) {
                image.setRGB(col, row, pic.image.getRGB(col, row));
            }
        }
    }
    ...
    public boolean equals(final Object other) {
        ...
        for (int col = 0; col < width(); col++) {
            for (int row = 0; row < height(); row++) {
                if (this.getRGB(col, row) != that.getRGB(
                            col, row)) {
                    return false;
                }
            }
    ...
    public void actionPerformed(final ActionEvent e) {
        FileDialog chooser
            = new FileDialog(frame, "Use a .png or .jpg extension",
                             FileDialog.SAVE);
